...
import {  signOut,signInWithEmailAndPassword,onAuthStateChanged,createUserWithEmailAndPassword, getAuth,  FacebookAuthProvider , signInWithPopup, GoogleAuthProvider , provider,OAuthProvider } from "firebase/auth";
...
// signup user 
// const signupUser1 = () =>{
//  createUserWithEmailAndPassword(auth, 'hello.eds@asd' , 'hello9090' ).then((value)=> console.log(value));
...
// const fbprovider = new OAuthProvider('apple.com');
...
// const signupUser = () =>{
//     createUserWithEmailAndPassword(auth , "disha.sadf@gamil.com", "pituye" );
// };
...
const [email,setemail] = useState("");
  ...
  const signin = () =>{
    ...
    {/* for the signup with email and password  */}
    {/* <div>
      <h1>email and password</h1>
      <button onClick={signupUser1}> resister </button>
    </div>
...
{/* 
            ...
            <input    onChange={e => setemail(e.target.value)} value={email}   type="email" required placeholder='ENTER YOUR EMAIL ' className='bg-violet-200 rounded-full '/>
            <label htmlFor="">Password</label>
            <input   value={password} onChange={e => setpassword(e.target.value)} type="password" placeholder=' enter your password '  className='bg-violet-200 rounded-full '   />
      
            <button className='bg-red-200' onClick={createUser} > signup  </button>
      ...
      <button className='bg-red-200' onClick={signin} > signin </button>
...
// AFTER THIS YOU Can remove the all code 
  ...
  // THIS IS FOR THE DATABASE REALTIME
  const putData = ( ) =>{
    set(ref(db,"USER/piyush"),{id:1 , name:"hOLA  ", age: 21 , });
  };
